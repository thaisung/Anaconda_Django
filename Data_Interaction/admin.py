...
from .models import User,CategoryProduct,ListProduct,AdminInformation,BankInformation,PersonalTransactionHistory
...
class UserAdmin(BaseUserAdmin):
    list_display = ('id','email', 'username','Money','Total_recharge_money','Total_amount_deducted','date_joined','last_login','OTP','is_active',)
    search_fields = ('email', 'username','Money','Avatar','OTP','date_joined','last_login',)

    fieldsets = BaseUserAdmin.fieldsets
    fieldsets[0][1]['fields'] = fieldsets[0][1]['fields'] + (
        'Money','Total_recharge_money','Total_amount_deducted','Avatar','OTP'
    )
...
class ListProductAdmin(admin.ModelAdmin):
    list_display = ('id','Name','Price','Nation','Category','Active')
...
class BankInformationAdmin(admin.ModelAdmin):
    list_display = ('id','Account_Name','Account_Number','Bank_Name','Short_Name','QR_Code_Link','Main_Color','Avatar','Api_Key','Code_Api_Key','Active')
